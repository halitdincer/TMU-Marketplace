...
function AdCard({ ad }) {
  const shortDescription = ad.description.length > 100 ? `${ad.description.slice(0, 100)}...` : ad.description;
    ...
    <article className="max-w-md mx-auto bg-white border border-gray-200 shadow-lg rounded-lg overflow-hidden my-5">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between text-xs text-gray-600 mb-2">
          ...
          <span className="rounded-full bg-gray-100 px-3 py-1">{ad.category || 'No Category'}</span>
        </div>

        <h3 className="text-lg font-semibold text-gray-800 hover:text-gray-600 transition-colors duration-200">
          <a href="#" className="stretched-link">{ad.title || 'No Title'}</a>
        </h3>
